...
class TestSelectorErrors:
    def test_invalid_selector_raises(self, tmp_path):
        ...
        with pytest.raises(SelectorError, match="No element matches selector"):
            nokap.webshot(html_file, out, selector="#nonexistent")

    def test_selector_error_is_nokap_error(self, tmp_path):
        html_file = tmp_path / "test.html"
        html_file.write_text("<html><body><p>Hello</p></body></html>")
        out = tmp_path / "test.png"
        with pytest.raises(NokapError):
            nokap.webshot(html_file, out, selector=".does-not-exist")
    ...
    def test_empty_element_ignored_in_union(self, tmp_path):
        ...
        out = tmp_path / "test.png"
        nokap.webshot(html_file, out, selector=[".target"])
        assert out.exists()
...
class TestNavigationErrors:
    def test_navigation_nonexistent_file_raises(self, tmp_path):
        ...
        with pytest.raises(NavigationError, match="ERR_FILE_NOT_FOUND"):
            nokap.webshot("file:///nonexistent/path/to/file.html", out)
...
class TestParameterValidation:
    def test_selector_and_cliprect_mutually_exclusive(self, tmp_path):
        ...
        with pytest.raises(ValueError, match="Cannot specify both"):
            nokap.webshot(html_file, out, selector="p", cliprect=(0, 0, 100, 100))


class TestEdgeCases:
    def test_webp_format(self, tmp_path):
        out = tmp_path / "test.webp"
        nokap.from_html("<h1>WebP Test</h1>", out)
        assert out.exists()
    ...
    def test_expand_single_int(self, tmp_path):
        ...
        out = tmp_path / "test.png"
        nokap.from_html(html, out, selector="#box", expand=10)
        assert out.exists()
    ...
    def test_large_html_table(self, tmp_path):
        ...
        nokap.from_html(html, out, selector="table")
        assert out.exists()
...
class TestPDFOptions:
    def test_pdf_landscape(self, tmp_path):
        ...
        nokap.from_html("<h1>Landscape</h1>", out, landscape=True)
        assert out.exists()
        assert out.read_bytes()[:4] == b"%PDF"

    def test_pdf_a4(self, tmp_path):
        out = tmp_path / "a4.pdf"
        nokap.from_html("<h1>A4</h1>", out, page_size="a4")
        assert out.exists()

    def test_pdf_print_background(self, tmp_path):
        html = "<html><body style='background:lightblue;'><h1>BG</h1></body></html>"
        out = tmp_path / "bg.pdf"
        nokap.from_html(html, out, print_background=True)
        assert out.exists()


class TestBrowserRecovery:
    def test_works_after_close(self, tmp_path):
        """Calling webshot after nokap.close() should auto-restart Chrome."""
        out1 = tmp_path / "first.png"
        nokap.from_html("<h1>First</h1>", out1)
        assert out1.exists()

        nokap.close()

        out2 = tmp_path / "second.png"
        nokap.from_html("<h1>Second</h1>", out2)
        assert out2.exists()
    ...
    def test_multiple_sequential_screenshots(self, tmp_path):
        ...
        for i in range(5):
            ...
            nokap.from_html(f"<h1>Shot {i}</h1>", out)
            assert out.exists()
